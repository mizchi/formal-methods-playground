...
共通フローの中で、AI と人間の owner を固定する。
ここを曖昧にすると、AI が出した反例を「仕様判断」と勘違いする。

| ステップ | AI の担当 | 人間の担当 |
| --- | --- | --- |
| 対象を選ぶ | 事故りやすい箇所をコード・docs から候補化する | 今回守りたい境界を決める |
| claim を抜く | 「許可」「禁止」「必ず」「いつか」を列挙する | どの claim を正として扱うか承認する |
| モデルを書く | 最小モデルと検査コマンドを作る | 抽象化で捨てた前提を確認する |
| 結果を読む | `SAT` / trace / proof failure をドメイン語に翻訳する | その反例が意図か bug か判断する |
| 修正先を出す | docs / code / model の修正候補を整理する | 何を正とし、どこを直すか決める |
| CI に載せる | 決定済みの性質を regression guard にする | その性質を今後の契約として受け入れる |

例えば、Z3 が次の witness を返したとする。

```text
role = Viewer
featureFlag(settings_preview) = true
reachable(Settings) = true
```

AI が出すべき結論は、次の形である。

```text
preview flag を持つ Viewer が設定画面に到達できます。
これは「管理者だけが設定画面を開ける」という docs と矛盾します。

確認したいこと:
  settings_preview は非管理者に設定画面を開かせる意図ですか?
  意図なら docs に preview 例外を書く必要があります。
  意図でないなら authorization bug として code を直す必要があります。
```

AI はここまでを担当する。
「preview は許してよい」という業務判断や、
「docs を直すのではなく code を直す」という優先順位は人間が決める。
